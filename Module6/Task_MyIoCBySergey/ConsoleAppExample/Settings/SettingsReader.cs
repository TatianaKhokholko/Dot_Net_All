...
namespace ConsoleAppExample.Settings
{
    public class SettingsReader : ISettingsReader
    {
        ...
        public object LoadSection(Type type)
        {
            ...
            var jsonFile = File.ReadAllText(_configurationFilePath);
            var section = IsTextModif(type.Name.Replace(_sectionNameSuffix, string.Empty));
        ...
        private static string IsTextModif(string text)=> string.IsNullOrWhiteSpace(text) 
            ? string.Empty: $"{text[0].ToString().ToLowerInvariant()}{text.Substring(1)}";
    }
}
